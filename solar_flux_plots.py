...
for i in tqdm(range(len(time))):
    ...
    # Ensure index is valid and within range
    if ind < len(em):
        data.iloc[i, data.columns.get_loc('dEM_dt')] = ((em[ind] - em[i]) / interval)/1e49  # (ΔEM / Δt)

# Define FAI conditions
DELTA_EM_THRESHOLD = 0.03  # Emission measure rate change threshold
...
# Plot Flux (short_flux & long_flux) and FAI vs. time
...
plt.ylim(0, 0.2e7)  # Set the y-axis range from 0 to 1e7
plt.legend()
plt.grid(True)
plt.show()
...
plt.title('Differential Emission Measure (DEM) vs. Time')
plt.legend()
plt.grid(True)
plt.show()
